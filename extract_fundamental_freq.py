...
def extract_fundamental_freq(sample, fs):
    ...
    amplitude = abs(spectrum_halved)
    peak_vec = []
    for i in range(len(amplitude)):     #leaving only highest peaks of spectrum
        if amplitude[i - 1] < amplitude[i] and amplitude[i + 1] < amplitude[i]:
            peak_vec.append(amplitude[i])
        else:
            peak_vec.append(0.0)
    ...
    freq_ratio = Fraction(peak_1_idx, peak_2_idx).limit_denominator(20)
    fundamental_idx = peak_1_idx / freq_ratio[0]
